...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/pushover/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/pushover/*")
            ...
            #    "--include=src/saltext/pushover/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/pushover/config/schemas",
    )
